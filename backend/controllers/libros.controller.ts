...
export class LibrosController {
  ...
  public borrarLibro = async (
    ...
    try {
      ...
      if (!salida) {
        const error = new Error(`El ID#${req.params.id} no existe para eliminar`);
        error.name = "404-IdLibro";
        throw error;
      }
      return res.status(codigo).json(salida);
